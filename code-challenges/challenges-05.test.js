...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  starWarsArr.sort((a, b) => {
    let aa = parseInt(a.height);
    let bb = parseInt(b.height);
    if (aa < bb) {
      return 1;
    }
    if (aa > bb) {
...
['Wlecome', 'elcome', 'lcome', 'come', 'ome', 'me', 'e', ''].
------------------------------------------------------------------------------------------------ */
...
const howMuchPencil = (str) => {
  ...
  for (let index = 0; index < str.length + 1; index++) {
    let removedItems = str.length;
    let elements = str.slice(index, removedItems, '');
...
/////// i try to solve this issue more 3 hours i comment my respond because id did not work  ///////
  ...
  // Solution code here...
   recipe.ingredients.forEach((item, index) => {
    ...
    result.forEach((item, index) => {
      ...
      let removedIndex1 = index + 1;
      // console.log(removedIndex1);
      let removedIndex2 = newIndex + removedIndex1;

  let resarr=item.slice(newIndex, removedIndex2);
      result2.push(resarr);
    });
  });
