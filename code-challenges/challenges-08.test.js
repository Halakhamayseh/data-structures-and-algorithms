...
const typeNum = (arr) => {
  // Solution code here...
  //   let regex = /[0-9]/g;
  //   let arrN = arr.filter(item => {
  //     if ( regex.test(item[0])) { return true; }
  //   });
  //   return arrN;
  // };
...
const containsAnd = (arr) => {
  ...
  let regex = /and/g;
     return arr.filter(item => {
       if (item.match(regex)) { return true; }
    });
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  /////i try a lot to solve it 
  // let nn = arr.map(item => {
  //   return nn;
  // });
  // for (let index = 0; index < forbiddenValues.length; index++) {
    //   let element = forbiddenValues[index];
  ...
  arr.filter(item => {
     ...
     return notInFirstArray.forEach(elem => {
      if (item !== elem) {
        nArr.push(elem);
      }
  });
